...
def beginner_explain(row: dict[str, Any], regime: str = "중립") -> dict[str, str]:
    ...
    action_guide = {
        ...
        "분할관심": f"초보 가이드: {name}은(는) 나눠 사기만 검토하세요. 손절 {int(row.get('risk', {}).get('stop_price') or 0):,}원 지키며 소액→추가.",
        "소액관심": f"초보 가이드: 시장이 약해요. {name}은(는) 소액만, 손절 {int(row.get('risk', {}).get('stop_price') or 0):,}원 필수예요.",
        "돌파대기": f"초보 가이드: {name}은(는) 고점 돌파+거래량 순간을 노리세요. 들어가면 손절 {int(row.get('risk', {}).get('stop_price') or 0):,}원 / 1차익절 {int(row.get('risk', {}).get('take1_price') or 0):,}원.",
