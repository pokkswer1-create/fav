...
with st.expander("초보자를 위한 읽는 법", expanded=True):
    st.markdown(
        """
1. **액션**만 먼저 보세요: `매수관심` / `관망` / `관심목록` / `회피`
2. **왜?** 칸은 쉬운 말로 적혀 있어요. (큰손 돈, 가격이 아직 안 올랐는지, 돌파 여부)
3. **과거 성적**은 비슷한 상황에서 손절·익절 규칙을 적용한 *참고* 숫자예요. 미래 보장이 아닙니다.
4. **매수관심**은 ‘셋업 + 고점 돌파’가 겹칠 때만 나옵니다. 돌파 전이면 보통 `관망`입니다.
"""
    )
